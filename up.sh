...
case "$1" in
  ...
  2|Tutor|Seq|Sequential)
	...
	docker run -d -p 8082:8080 --name=air_webserver_2 -v  $(pwd)/dags:/usr/local/airflow/dags saastoolset/sqlg-airflow:latest webserver
	# sleep 20
  ...
  3|Example)
	...
	docker run -d -p 8083:8080 --name=air_webserver_3 -e AIRFLOW__CORE__LOAD_EXAMPLES=True -e AIRFLOW__WEBSERVER__RBAC=False -v  $(pwd)/dags:/usr/local/airflow/dags saastoolset/sqlg-airflow:latest webserver
	# sleep 20
